...
public class MyController2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("all".equals(type)) comm = new ListCommand();
			
		else if ("dept".equals(type)) comm = new DeptCommand();

		else if ("deptList".equals(type)) comm = new DeptListCommand();
		
		else if ("fullname".equals(type)) comm = new FullnameCommand();
		
		else if ("fullnameList".equals(type)) comm = new FullnameListCommand();
		
		else if ("search".equals(type)) comm = new SearchCommand();
