...
<body>
	...
	<c:forEach var="listItem" items="${alist }"
	 begin="0" end="${alist.size() }" step="1" varStatus="status">
	<p>
		var 값 : ${listItem }<br> 
		current : ${status.current }<br>		
		count : ${status.count }<br> <%--처리되는 데이터 건수 --%>
		index : ${status.index }<br> <%--데이터의 인덱스 값 --%>
		first : ${status.first }<br> <%--첫번째 데이터 여부(true or false) --%>
		last : ${status.last }<br> <%--마지막 데이터 여부(true or false) --%>
