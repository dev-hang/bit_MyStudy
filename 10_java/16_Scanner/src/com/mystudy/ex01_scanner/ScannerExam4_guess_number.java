...
public class ScannerExam4_guess_number {
	...
	public static void main(String[] args) {
		...
		// 5번까지 시도해서 맞추면 : 성공!!! n번 만에 맞췄습니다.(화면출력)
		...
		while (oneMore) {
			...
			while (true) {
				...
				if (cnt <= 0) {
					System.out.println("실패~~~ 내가 생각한 숫자는 " + comNum + "입니다.");
					break;
				}
				
				//3. 숫자값 비교
				if (userNum > comNum) {
					System.out.println(userNum + "보다 더 작은 수입니다");
				}
				if (userNum < comNum) {				
					System.out.println(userNum + "보다 더 큰 수입니다");
				}
				if (userNum == comNum) {
					System.out.println("성공!!! " + tryCnt + "번 만에 맞췄습니다.");
					break;
				}
