...
class ATM extends Thread {
	...
	void add(int money) {
		synchronized(bank) {
			bank.money += money;
			System.out.println(Thread.currentThread() + " 입금: " + money + ", 통장잔액: " + bank.money);
		}
	}
	//출금
	void out(int money) {
		synchronized(bank) { //객체에 대한 동기화
			bank.money -= money;
			System.out.println(Thread.currentThread() + " 출금: " + money + ", 통장잔액: " + bank.money);
		}
	}
...
public class ATM_ThreadError2_Sync {
	...
	public static void main(String[] args) {
		...
		try {
			//main 쓰레드에 연결처리
			atm1.join(); //atm1 종료 후 main 쓰레드 종료 처리
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
