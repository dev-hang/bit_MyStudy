...
SELECT * FROM ORDERS WHERE CUSTID = 1;
--CUSTOMER 테이블과 ORDERS 테이블 데이터 동시 조회
SELECT * 
...
SELECT *
    FROM CUSTOMER C 
                 INNER JOIN ORDERS O -- INNER 생략 가능
...
--====================================
--(문제해결) 테이블 조인을 통해서 문제 해결(단일 SELECT문을 사용 해결)
--실습 : '야구를 부탁해'라는 책이 팔린 내역을 확인(책제목, 판매금액, 판매일자)
SELECT B.BOOKNAME "책제목" , O.SALEPRICE "판매금액", O.ORDERDATE "판매일자"
...
SELECT O.SALEPRICE "책값", O.ORDERDATE  "구입일자"
    FROM CUSTOMER C, ORDERS O
  WHERE C.CUSTID = O.CUSTID
       AND C.NAME = '추신수';
       
--실습 : '추신수'가 구입한 합계금액을 확인
SELECT SUM(O.SALEPRICE) "합계금액"
    FROM CUSTOMER C, ORDERS O
  WHERE C.CUSTID = O.CUSTID
       AND C.NAME = '추신수';
       
--실습 : 박지성, 추신수가 구입한 내역 확인(이름, 판매금액, 판매일자)
SELECT C.NAME "이름", O.SALEPRICE "판매금액", O.ORDERDATE "판매일자"
    FROM CUSTOMER C, ORDERS O
  WHERE C.CUSTID = O.CUSTID
