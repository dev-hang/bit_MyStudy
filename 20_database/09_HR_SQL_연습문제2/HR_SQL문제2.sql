...
****************************************************/
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME, SALARY, JOB_TITLE, COMMISSION_PCT
...
********************************/    
SELECT DEPARTMENT_ID, TO_CHAR(SUM(SALARY), '$999,999') "급여 합계", 
...
*****************************/
SELECT 'HR-Company' AS COMPANY_NAME, FIRST_NAME || ' ' || LAST_NAME AS NAME, 
       JOB_ID, DEPARTMENT_NAME, CITY
  FROM EMPLOYEES E, DEPARTMENTS D, LOCATIONS L
 WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID
   AND L.LOCATION_ID = D.LOCATION_ID
   AND L.CITY = INITCAP('OXFORD');
...
SELECT EMPLOYEE_ID, SALARY, GRADE_LEVEL, LOWEST_SAL, HIGHEST_SAL
  FROM EMPLOYEES E, JOB_GRADES G
  WHERE SALARY BETWEEN LOWEST_SAL AND HIGHEST_SAL
ORDER BY EMPLOYEE_ID, GRADE_LEVEL
;

SELECT E.FIRST_NAME || ' ' || E.LAST_NAME AS NAME, E.JOB_ID, D.DEPARTMENT_NAME, E.HIRE_DATE, E.SALARY, GRADE_LEVEL
  FROM EMPLOYEES E, DEPARTMENTS D, JOB_GRADES G
 WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID
   AND SALARY BETWEEN LOWEST_SAL AND HIGHEST_SAL
;
--
SELECT EMPLOYEE_ID, FIRST_NAME || ' ' || E.LAST_NAME AS NAME, E.JOB_ID, D.DEPARTMENT_NAME, E.HIRE_DATE, E.SALARY,
       (SELECT GRADE_LEVEL FROM JOB_GRADES WHERE E.SALARY BETWEEN LOWEST_SAL AND HIGHEST_SAL) AS GRADE
  FROM EMPLOYEES E, DEPARTMENTS D
 WHERE E.DEPARTMENT_ID = D.DEPARTMENT_ID;
