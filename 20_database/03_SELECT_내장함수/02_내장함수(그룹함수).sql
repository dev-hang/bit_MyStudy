...
COUNT ( * ) : 데이터 개수 조회(전체 컬럼에 대하여)
COUNT (컬럼명) : 데이터 개수 조회(지정된 컬럼만 대상으로)
SUM (컬럼) : 합계 값 구하기
AVG (컬럼) : 평균 값 구하기
MAX (컬럼) : 최대값 구하기
MIN (컬럼) : 최소값 구하기
...
SELECT COUNT(PHONE) FROM CUSTOMER; --4 조회 : NULL 값은 개수에서 제외됨
...
--MAX (컬럼) : 컬럼값 중 최대값 구하기
--MIN (컬럼) : 컬럼값 중 최소값 구하기
...
-- (실습) 
-- 1. 박지성의 총 구매액(CUSTID : 1)
--2. 박지성이 구매한 도서의 수(COUNT)
--3. 성이 '김'씨인 고객의 이름과 주소
--4. 성이 '박'씨이고 이름이 '성'으로 끝나는 고객 이름과 주소
